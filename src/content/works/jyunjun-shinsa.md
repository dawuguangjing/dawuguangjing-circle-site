---
title: "従順審査"
catch: "あなたは男尊女卑常識時代における、従順審査の審査員です。従順審査の名のもと、美女を好き放題しましょう。"
shortDescription: "従順審査シミュレーションゲーム。"
metaDescription: "「従順審査」は男尊女卑が常識化した世界で審査員として美女を審査する同人シミュレーションゲーム。CG70枚超・マルチエンド収録。DLsite・FANZAで販売中。体験版あり。"
releaseDate: 2026-02-03
isR18: true
platformLinks:
  fanza: "https://al.fanza.co.jp/?lurl=https%3A%2F%2Fwww.dmm.co.jp%2Fdc%2Fdoujin%2F-%2Fdetail%2F%3D%2Fcid%3Dd_730295%2F&af_id=dawuguangjin-001&ch=search_link&ch_id=link"
  dlsite: "https://dlaf.jp/aix/dlaf/=/t/s/link/work/aid/dawuguangjing/id/RJ01562523.html"
trialLinks:
  fanza: "https://sample9.dmm.co.jp/digital/cg_game/d_730295/d_730295t.zip"
  dlsite: "https://trial.dlsite.com/doujin/RJ01563000/RJ01562523_trial.zip?_gl=1*gwbexe*_gcl_au*MTc3NzkxNzk0Ni4xNzY3MjUwMDQ5LjE5ODM0ODYxODQuMTc2OTA3NTc0MC4xNzY5MDc1NzY4"
trialNote: "体験版では南雲姫華ルートをお楽しみいただけます。"
environments:
  windows: true
  mac: false
  browserPc: true
  browserMobileBeta: true
  ios: false
  android: false
volume:
  playTimeMin: 90
  cgCount: 70
  hSceneCount: 50
  branching: "multi"
aiUsage:
  level: "partial"
  noteShort: "一部のグラフィック素材にAIを使用しています。"
sellingPoints:
  - "従順審査員として美女を好き放題。 — シナリオ分岐・4エンドの選択式"
  - "70枚超のオリジナルCG収録"
  - "体験版あり — お気軽にお試しください！"
images:
  cover: "https://storage.googleapis.com/dawuguangjing-assets/jyuujyun-shinsa-1.jpg"
  screenshots:
    - "https://storage.googleapis.com/dawuguangjing-assets/jyuujyun-shinsa-1-slide1.jpg"
    - "https://storage.googleapis.com/dawuguangjing-assets/jyuujyun-shinsa-1-slide2.jpg"
    - "https://storage.googleapis.com/dawuguangjing-assets/jyuujyun-shinsa-1-slide3.jpg"
    - "https://storage.googleapis.com/dawuguangjing-assets/jyuujyun-shinsa-1-slide4.jpg"
    - "https://storage.googleapis.com/dawuguangjing-assets/jyuujyun-shinsa-1-slide5.jpg"
                
---

## ストーリー

戦勝国A国は「平等」を看板にしてきた。しかし国内で格差や移民、治安への不満が増えるほど、その看板は弱くなる。
そこでA国は外に‘反面教師’を用意した。差別が目に見える国を作り、「それでも我が国はましだ」と世論をまとめ直すために。

A国が我が国へ持ち込んだのは、支援と協力を名目にした「共同社会研究協定」。
「雇用の安定」「家庭の再建」「分断の抑止」――耳あたりのいい言葉で、反対の声を鈍らせた。

だが中身は、男尊女卑加速化計画だった。
差別を法律ではなく‘慣行’として根づかせ、日常に溶かし、抵抗が起きにくい形へ変えていく。
その過程を観測し、数字と映像にして、A国の宣伝に使う――救うためではない。比べるためだ。

学校は「適性」、メディアは「秩序」。自治体は「家庭支援」、企業は「合理化」。どれも「守るため」の顔をして、静かに進んでいった。

時は現代。就職で、決定的な楔が打ち込まれる。新卒採用で、女性だけが提出を求められる証明書――従順審査合格票。
能力ではなく、反論しない態度、沈黙の作り方を‘指数’として測る審査だ。提出は「任意」。だが出さなければ、選考に進めない。

落ちた者に理由は告げられず、異議申立てもできない。三年で大学は対策講座を開き、親は娘に言う。「落ちると、一生ものの傷になる」

卒業まで、あと一か月。学内の掲示板に、薄紫の紙が一枚貼られている。

――『従順審査 実施通知 対象者:女子学生 全員』

## シチュエーション

あなたはこの時代における、従順審査の審査員です。従順審査の名のもと、美女を好き放題しましょう。

## この製品について

製品版は森宮花音、佐藤真希、南雲姫華の3ルートプレイできます。（各種4エンド）
体験版では南雲姫華ルートをお楽しみいただけます。

## リメイク版について

本作をシリーズ最高クオリティで作り直した[「従順審査１」](../jyunjun-shinsa-1/)を FANZA にて配信中です。
完全新規イベントCGを追加し、基本CG95枚＋・ムービー57本＋を収録しています。

## レビューについて

もしお気に入りいただけましたら、
短い一言でも感想をいただけると励みになります。
